...
async def query(req: QueryRequest):
    ...
    # 2. Query 理解（改写 + 分解）
    t_qu = time.time()
    qu_result = get_understander().understand(question, history)
    search_query = qu_result["rewritten"]
    qu_ms = (time.time() - t_qu) * 1000
    logger.info(f"[Query理解] {qu_ms:.0f}ms | intent={qu_result['intent']}")

    # 3. 混合检索（用改写后的问题检索，内部已有详细计时日志）
    context_chunks = []
    try:
        context_chunks = get_retriever().retrieve(search_query)
    except Exception as e:
        logger.error(f"检索失败: {e}")
        log_query(question, status="retrieve_error", error=str(e), session_id=session_id,
    ...
    logger.info(f"[全链路] 总耗时={total_ms:.0f}ms | 记忆={mem_ms:.0f}ms | Query理解={qu_ms:.0f}ms | 生成={gen_ms:.0f}ms")
...
async def query_stream(req: QueryRequest):
    ...
    search_query = qu_result["rewritten"]
    qu_ms = (time.time() - t_qu) * 1000
    logger.info(f"[SSE-Query理解] {qu_ms:.0f}ms | intent={qu_result['intent']}")

    # 3. 检索（用改写后的问题，内部已有详细计时）
    context_chunks = []
    try:
        context_chunks = get_retriever().retrieve(search_query)
    except Exception as e:
    ...
    async def event_stream():
        for chunk_type, data in generator.generate_stream(question, context_chunks, history):
            if chunk_type == "source":
                ...
                logger.info(f"[SSE-全链路] 总耗时={total_ms:.0f}ms | 记忆={mem_ms:.0f}ms | Query理解={qu_ms:.0f}ms | 生成={gen_ms:.0f}ms")
                # 记录历史
