...
# ── 10 道测试题（来自工单） ──
_SESSION_ID = f"eval_{int(time.time())}"  # 每次评测独立 session
...
def call_rag(question: str, port: int = 8004) -> Dict[str, Any]:
    ...
    payload = {"question": question, "session_id": _SESSION_ID}
...
def run_evaluation(port: int = 8004, runs: int = 3) -> Dict[str, Any]:
    ...
    # 清空旧 session 历史，避免跨次累积
    import httpx as _httpx
    try:
        _httpx.post(f"http://localhost:{port}/session/clear?session_id={_SESSION_ID}", timeout=5)
    except Exception:
        pass
